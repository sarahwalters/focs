...
let minus = ("minus", "/m./n.n pred m") (* http://safalra.com/lambda-calculus/integer-arithmetic/ *)
(* think more about why b pred a is a-b *)
let geq = ("geq","/m./n.iszero (minus n m)")

let eq = ("eq","/m./n.and (geq m n) (geq n m)")

let pair = ("pair","/m./n./p.p m n") (* https://en.wikipedia.org/wiki/Church_encoding#Church_pairs *)

let match_pair = ("match_pair","/pair./fun.pair fun") (* this seems unnecessary *)

let fst = ("fst","/pair.pair true")

let snd = ("snd","/pair.pair false")
(* think more about why fst and snd work *)
let update_fst = ("update_fst","/oldpair./newfst.pair newfst (snd oldpair)")

let update_snd = ("update_snd","/oldpair./newsnd.pair (fst oldpair) newsnd")
...
let neg_int = ("neg_int","/int.pair (not (fst int)) (snd int)")

let plus_int = ("plus_int","/int1./int2.if (fst int1)
                                          (
                                            if (fst int2)
                                              (int (plus (snd int1) (snd int2)))
                                              (
                                                if (geq (snd int1) (snd int2))
                                                  (int (minus (snd int1) (snd int2)))
                                                  (neg_int (int (minus (snd int2) (snd int1))))
                                              )
                                          )(
                                            if (fst int2)
                                              (
                                                if (geq (snd int1) (snd int2))
                                                    (neg_int (int (minus (snd int1) (snd int2))))
                                                    (int (minus (snd int2) (snd int1)))
                                              )
                                              (neg_int (int (plus (snd int1) (snd int2))))
                                          )")

let times_int = ("times_int","/int1./int2.if (or
                                              (and (fst int1) (fst int2))
                                              (and (not (fst int1)) (not (fst int2)))
                                            )
                                              (int (times (snd int1) (snd int2)))
                                              (neg_int (int (times (snd int1) (snd int2))))")
...
let is_empty = ("is_empty","/list.fst list")

let cons = ("cons","/h./t.pair false (pair h t)") (* https://en.wikipedia.org/wiki/Church_encoding#List_encodings*)

let head = ("head","/list.snd list true")

let tail = ("tail","/list.snd list false")

let match_list = ("match_list","/list./a./f.if (is_empty list)
                                              (a)
                                              (f (head list) (tail list))")

let length = ("length","Y (/length./list.if (is_empty list) _0 (succ (length (tail list))))")

let sum = ("sum","Y (/sum./list.if (is_empty list) _0 (plus (head list) (sum (tail list))))")

let append = ("append","Y (/append./list1./list2.if (is_empty list1)
                                                   list2
                                                   (cons (head list1) (append (tail list1) list2)))")

let map = ("map","Y (/map./f./list.if (is_empty list) empty (cons (f (head list)) (map f (tail list))))")

let q3_defs = q2_defs @ [empty; is_empty; cons; head; tail; match_list; length; sum; append; map]
